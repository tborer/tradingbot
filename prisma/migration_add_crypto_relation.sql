-- This migration adds the missing crypto relation field to MicroProcessingSettings table

-- First, backup existing data
CREATE TABLE IF NOT EXISTS "MicroProcessingSettings_Backup_Relation" AS 
SELECT * FROM "MicroProcessingSettings";

-- Drop the existing table
DROP TABLE IF EXISTS "MicroProcessingSettings";

-- Recreate the table with proper constraints and the crypto relation field
...
SELECT b.* FROM "MicroProcessingSettings_Backup_Relation" b
...
BEGIN
  SELECT COUNT(*) INTO backup_count FROM "MicroProcessingSettings_Backup_Relation";
