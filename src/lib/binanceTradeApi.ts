 ...
 * 
 * IMPORTANT: Both the query string (containing all parameters) AND the secret key are required
 * to generate a valid signature. The secret key is never sent in the request but is used
 * server-side to create the signature.
 */
 ...
 * 2. Generating an HMAC SHA256 hash of this string using the secret key as the key for the HMAC operation
 * 3. Appending the signature to the query string
 * 
 * IMPORTANT: The signature is a critical security component that proves the request is authentic.
 * It requires BOTH the complete set of parameters AND the user's secret API key to generate.
 * The secret key is never transmitted but is used server-side to create the signature.
 * 
