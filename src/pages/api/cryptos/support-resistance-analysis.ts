...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  const requestId = `support-resistance-analysis-${Date.now()}-${Math.random().toString(36).substring(2, 10)}`;
  
  // Log request details
  console.log(`[${requestId}] Support/resistance analysis request started for symbol: ${req.body?.symbol || 'unknown'}`);
  ...
  try {
    const logEntry = await prisma.apiLog.create({
      data: {
        requestId,
        endpoint: '/api/cryptos/support-resistance-analysis',
  ...
  if (req.method !== 'POST') {
    const errorDetails = createAndLogError(
      ...
      4000,
      `Method ${req.method} not allowed for support/resistance analysis`,
  ...
  try {
    ...
    if (!user) {
      const errorDetails = createAndLogError(
        ...
        ErrorSeverity.ERROR,
        4001,
        'Unauthorized access to support/resistance analysis API',
        { requestId }
      );
    ...
    if (!symbol) {
      const errorDetails = createAndLogError(
        ...
        ErrorSeverity.ERROR,
        4002,
        'Missing symbol in support/resistance analysis request',
    ...
    console.log(`[${requestId}] Using CryptoHistoricalData table directly for support/resistance analysis of ${normalizedSymbol}`);
    
    console.log(`[${requestId}] Processing support/resistance analysis for symbol: ${symbol}`);
    ...
    // Fetch and analyze support/resistance data
    console.log(`[${requestId}] Fetching and analyzing support/resistance data for ${symbol}`);
    ...
    if (!analysis) {
      const errorDetails = createAndLogError(
        ...
        ErrorSeverity.ERROR,
        4007,
        `Failed to analyze support/resistance for ${symbol}`,
        { requestId, symbol }
      );
        ...
        try {
          await prisma.apiLog.update({
            ...
            data: {
              status: 'ERROR',
              statusCode: 500,
              responseBody: JSON.stringify({ error: 'Failed to analyze support/resistance', code: errorDetails.code }),
              errorMessage: `Failed to analyze support/resistance for ${symbol}`,
              endTime: new Date(),
              duration: Date.now() - startTime
            }
          });
      ...
      return res.status(500).json({ error: 'Failed to analyze support/resistance', code: errorDetails.code });
    }

    // Log success
    const duration = Date.now() - startTime;
    console.log(`[${requestId}] Support/resistance analysis completed successfully for ${symbol} in ${duration}ms`);
    ...
    const errorDetails = createAndLogError(
      ...
      ErrorSeverity.ERROR,
      4005,
      `Error analyzing support/resistance: ${errorMessage}`,
      { 
        requestId, 
    ...
    console.error(`[${requestId}] Error in support/resistance analysis:`, error);
